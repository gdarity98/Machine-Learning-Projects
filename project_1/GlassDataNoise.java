...
public class GlassDataNoise {
	...
	private double[] rangeMin = {1.5112,10.73,0,0.29,69.81,0,5.43,0,0};
	private double[] rangeMax = {1.5339,17.38,4.49,3.50,75.41,6.21,16.19,3.15,0.51};
	
	public GlassDataNoise(String fullData) {
      ...
      //Making noise 10%.
  		...
  		for(double feat : features) {
  			features[i] = setNoise(i,randomArray,feat,rangeMin[i],rangeMax[i]);
	...
	public double setNoise(int check, int[] arrayToCheck, double change,double rangeMin, double rangeMax) {
		if(arrayToCheck[check] == 7) {
			Random r = new Random();
			double randomNumNoise = rangeMin + (rangeMax - rangeMin) * r.nextDouble();
			change = randomNumNoise;
			return change;
		}else {
			return change;
		}
